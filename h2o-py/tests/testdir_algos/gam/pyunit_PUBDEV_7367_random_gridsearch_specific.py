...
class test_gam_gridsearch:
    ...
    random_hyper_params = {'alpha': [0.1, 0.5], 'lambda':[0, 0.01], 'scale': [[1, 1], [2, 2], [0.01], [0.05]], 
                           'gam_columns': [["C6", "C7"], ["C6"]], 'num_knots': [[5, 5], [5]]};
    ...
    def train_models(self):
        self.h2o_model = H2OGridSearch(H2OGeneralizedAdditiveEstimator(family="multinomial",
                                                                       keep_gam_cols=True), hyper_params=self.random_hyper_params, search_criteria=self.search_criteria)
        self.h2o_model.train(x = self.myX, y = self.myY, training_frame = self.h2o_data)
        for model in self.h2o_model.models:
            print(model.actual_params['scale'], model.actual_params['gam_columns'], model.actual_params['num_knots'])
        print("done")
...
if __name__ == "__main__":
    h2o.init(ip='192.168.1.2', port=54321, strict_version_check=False)
